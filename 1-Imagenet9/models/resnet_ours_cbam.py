...
class BasicBlock(nn.Module):
    ...
    #
    #     assert not self.cbam is None
    #     out_causal, attn1, attn2 = self.cbam(out, return_attn=True)
    #     out_spurious = (1-attn1) * (1-attn2) * out
    #     out_causal += residual
    #     out_causal = self.relu(out_causal)
    #
    #     if self.split:
...
class ResNet(nn.Module):
    def __init__(self, block, layers,  network_type, num_classes, att_type=None, split_num=2):
        self.inplanes = 64
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, att_type=att_type, split_num=split_num)
...
def ResidualNet(network_type, depth, num_classes, att_type):
    ...
    if depth == 18:
        ...
        model = ResNet(BasicBlock, [3, 4, 6, 3], network_type, num_classes, att_type, split_num=3)
