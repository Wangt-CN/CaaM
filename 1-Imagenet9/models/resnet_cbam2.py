...
class ResNet_Feature(nn.Module):
    def __init__(self, block, layers,  network_type, att_type=None):
        self.inplanes = 64
...
def resnet18_feature(pretrained=False, **kwargs):
    ...
    """
    model = ResNet_Feature(BasicBlock, [2, 2, 2, 2], "ImageNet", "CBAM")
    return model

def resnet34_feature(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_Feature(BasicBlock, [3, 4, 6, 3], "ImageNet", "CBAM")
    return model
