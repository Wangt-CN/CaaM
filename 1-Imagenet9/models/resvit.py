...
class ViTResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, BATCH_SIZE_TRAIN=128, dim=128, num_tokens=16, mlp_dim=256, heads=8, depth=6,
        ...
        # Tokenization
        self.token_wA = nn.Parameter(torch.empty(self.cT, self.L), requires_grad=True)  # Tokenization parameters
    ...
    def forward(self, img, mask=None):
        ...
        # A = A.softmax(dim=-1)
        A = torch.einsum('bij,jk->bik', x, self.token_wA)
        A = A.softmax(dim=-2) # HW
...
class ViTResNet_Feature(nn.Module):
    def __init__(self, block, num_blocks, BATCH_SIZE_TRAIN=128, dim=128, num_tokens=16, mlp_dim=256, heads=8, depth=6,
                 emb_dropout=0.1, dropout=0.1):
        super(ViTResNet_Feature, self).__init__()
        self.in_planes = 64
        self.L = num_tokens
        self.cT = dim
        ...
        self.token_wA = nn.Parameter(torch.empty(self.cT, self.L), requires_grad=True)  # Tokenization parameters
        torch.nn.init.xavier_uniform_(self.token_wA)
        # self.token_wV = nn.Parameter(torch.empty(BATCH_SIZE_TRAIN, 256, self.cT),
        #                              requires_grad=True)  # Tokenization parameters
        # torch.nn.init.xavier_uniform_(self.token_wV)
...
class ViTResNet_Classifier(nn.Module):

    def __init__(self, block, num_classes=1000, bias=True):
        super(ViTResNet_Classifier, self).__init__()
...
def resvit18(**kwargs):
    model = ViTResNet(BasicBlock, [2, 2, 2, 2], num_classes=10)
    return model


def resvit18_feature(**kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ViTResNet_Feature(BasicBlock, [2, 2, 2, 2], BATCH_SIZE_TRAIN=32, **kwargs)
    return model


def classifier(**kwargs):
    classifier_model = ViTResNet_Classifier(BasicBlock, **kwargs)
