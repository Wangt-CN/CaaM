...
from dataset_imagenet import get_imagenet_dataloader, get_imagenet_dataloader_env, get_pre_dataloader, get_bias_dataloader
...
if __name__ == '__main__':
    ...
    train_loader_init = get_imagenet_dataloader(config['train_root'], batch_size=training_opt['batch_size'], train=True)
    ...
    if 'env_type' in variance_opt and variance_opt['env_type'] in ['auto-baseline', 'auto-iter', 'auto-iter-cluster']:
        if variance_opt['env_type'] == 'auto-iter-cluster':
            ...
            pre_train_loader, pre_optimizer, pre_schedule = get_pre_dataloader(config, config['train_root'], batch_size=512, train=True, num_workers=args.worker, n_env=variance_opt['n_env'])
    ...
    val_loaders = {}
    val_loaders['biased'] = get_imagenet_dataloader(config['val_root'],
                                                    batch_size=training_opt['val_batch_size'],
                                                    train=False)
    val_loaders['unbiased'] = get_imagenet_dataloader(config['val_root'],
                                                      batch_size=training_opt['val_batch_size'],
                                                      train=False)
    val_loaders['imagenet-a'] = get_imagenet_dataloader(config['imageneta_root'],
                                                        batch_size=training_opt['val_batch_size'],
                                                        train=False,
                                                        val_data='ImageNet-A')
    ...
    txt_write.write('Imageneta-best: '+str(best_acc_imageneta))
    txt_write.close
    writer.close()
