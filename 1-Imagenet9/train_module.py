...
def train_env_ours(epoch, net, train_loader, args, training_opt, variance_opt, loss_function, optimizer, warmup_scheduler):
    ...
    for batch_index, data_env in enumerate(train_loader[0]):
        ...
        if variance_opt['erm_flag']:
            images_erm, labels_erm, _ = data_env_erm
            if args.gpu:
        ...
        if variance_opt['sp_flag']:
            ...
            # env_dic_nll_spurious.append(smooth_loss(spurious_outputs, training_opt['classes']))
            # train_nll_spurious = torch.stack(env_dic_nll_spurious).mean()
...
def auto_split(ref_model, pre_train_loader, pre_optimizer, pre_schedule, soft_split_all):
    ...
    for epoch in range(100):
        ...
        for data in pre_train_loader:
            ...
            soft_split = F.softmax(soft_split_all[idx], dim=-1)
            penalty = (soft_split * penalty.unsqueeze(-1) / soft_split.sum(0)).sum(0)
            irm_risk_list = []
            ...
            irm_risk_final = - 2e6 * torch.stack(irm_risk_list).mean()
            reg = torch.var(F.softmax(soft_split_all, dim=-1).sum(0)) + torch.prod(F.softmax(soft_split_all, dim=-1), 1).sum()
            '''
            ...
            # reg = 0.1 * torch.var(hard_split_all.sum(0)) # no need loss for 0, 1
        ...
        print('\rInitializing Env [%d/%d]  Loss: %.2f  IRM_Risk: %.2f  Reg: %.2f  Cnt: %d  Lr: %.2f'
            ...
            # print(soft_split.data)

        if epoch > 80 and cnt > 5 or epoch == 99: #debug
        # if epoch > 80 and cnt > 5 or epoch == 5: #debug
            ...
            # print(F.softmax(soft_split_all, dim=-1))
